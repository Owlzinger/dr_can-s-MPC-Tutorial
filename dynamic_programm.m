...
J_temp = T_delta; % Assign delta T to cost to go,还没加上游或者下游的时间

[acc_x, acc_y] = find(acc < u_min | acc > u_max); % Find which acc is over the limit
%<-3或>2的index,acc_x行,acc_y列
Ind_lin_acc = sub2ind(size(acc), acc_x, acc_y); % Find linear index
% 使用 sub2ind 函数将坐标转换为线性索引将(2,3)这个坐标变成8(matrix中线性序号是从上到下从左到右排列的
% 即[1,4,7
% 2,5,8
% 3,6,9])所以,(2,3)是8
